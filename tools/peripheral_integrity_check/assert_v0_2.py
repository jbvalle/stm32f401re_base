...
def parse_header_file(header_file):
    ...
    struct_re = re.compile(r'\s*typedef\s+struct\s*(?:\{(.*?)\}|(\w+)\s*\{(.*?)\})\s*(\w+);', re.DOTALL)
    ...
    with open(header_file, 'r') as file:
        ...
        for struct_match in struct_re.finditer(content):
            unnamed_struct_body = struct_match.group(1)
            named_struct_name = struct_match.group(2)
            named_struct_body = struct_match.group(3)
            struct_name = struct_match.group(4)
            
            if unnamed_struct_body:
                struct_body = unnamed_struct_body
                struct_name = "unnamed_struct"
            else:
                struct_body = named_struct_body
